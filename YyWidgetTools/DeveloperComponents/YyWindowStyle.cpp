...
#include "YyTheme.h"
YyWindowStyle::YyWindowStyle(QStyle* style)
{
    _themeMode = eTheme->getThemeMode();
    connect(eTheme, &YyTheme::themeModeChanged, this, [=](YyThemeType::ThemeMode themeMode) { _themeMode = themeMode; });
}

YyWindowStyle::~YyWindowStyle()
{
}

void YyWindowStyle::drawPrimitive(PrimitiveElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const
{
    switch (element)
    {
    ...
    case QStyle::PE_PanelButtonTool:
    {
        if (option->state.testFlag(QStyle::State_Enabled))
        {
            ...
            {
                painter->setBrush(YyThemeColor(_themeMode, BasicPressAlpha));
            }
            else
            {
    ...
    case QStyle::PE_IndicatorArrowLeft:
    {
        ...
        painter->setRenderHint(QPainter::Antialiasing);
        painter->setPen(Qt::NoPen);
        painter->setBrush(option->state.testFlag(QStyle::State_Enabled) ? YyThemeColor(_themeMode, BasicText) : YyThemeColor(_themeMode, BasicTextDisable));
        // 左三角
        int sideLength = 10;
    ...
    case QStyle::PE_IndicatorDockWidgetResizeHandle:
    {
        painter->save();
        painter->setRenderHint(QPainter::Antialiasing);
        painter->setPen(QPen(YyThemeColor(_themeMode, BasicBaseLine), 2));
...
void YyWindowStyle::drawControl(ControlElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const
{
        ...
        if (const QStyleOptionTab* topt = qstyleoption_cast<const QStyleOptionTab*>(option))
        {
            ...
            if (topt->state & QStyle::State_Selected)
            {
                ...
                else
                {
                    ...
                    {
                        // 选中时覆盖
                        painter->setBrush(YyThemeColor(_themeMode, BasicSelectedHoverAlpha));
                    }
                    else
                    {
                        // 选中
                        painter->setBrush(YyThemeColor(_themeMode, BasicSelectedAlpha));
                    }
                }
            }
            else
