...
YyMicaBaseInitObject::YyMicaBaseInitObject(YyApplicationPrivate* appPrivate, QObject* parent)
    : QObject{parent}
{
    _appPrivate = appPrivate;
}

YyMicaBaseInitObject::~YyMicaBaseInitObject()
{
}

void YyMicaBaseInitObject::onInitMicaBase(QImage img)
{
    // QColorDialog
    // 统一处理为1920*1080以节省空间
    img = img.scaled(QSize(1920, 1080), Qt::IgnoreAspectRatio, Qt::SmoothTransformation);

    QImage blurImage = YyExponentialBlur::doExponentialBlur(img, 500).toImage();
