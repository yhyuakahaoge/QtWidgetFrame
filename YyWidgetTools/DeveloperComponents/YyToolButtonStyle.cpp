...
#include "YyTheme.h"
YyToolButtonStyle::YyToolButtonStyle(QStyle* style)
{
    _pIsSelected = false;
...
YyToolButtonStyle::~YyToolButtonStyle()
{
}

void YyToolButtonStyle::drawComplexControl(ComplexControl control, const QStyleOptionComplex* option, QPainter* painter, const QWidget* widget) const
{
    switch (control)
    {
        case QStyle::CC_ToolButton:
        {
            ...
            if (const QStyleOptionToolButton* bopt = qstyleoption_cast<const QStyleOptionToolButton*>(option))
            {
                ...
                painter->setPen(_pIsTransparent ? Qt::transparent : YyThemeColor(_themeMode, BasicBorder));
                // 背景绘制
                if (bopt->state.testFlag(QStyle::State_Enabled))
                {
                    if (bopt->state.testFlag(QStyle::State_Sunken))
                    {
                        painter->setBrush(_pIsTransparent ? YyThemeColor(_themeMode, BasicPressAlpha) : YyThemeColor(_themeMode, BasicPress));
                    ...
                    else
                    {
                        if (_pIsSelected)
                        {
                            painter->setBrush(_pIsTransparent ? YyThemeColor(_themeMode, BasicSelectedAlpha) : YyThemeColor(_themeMode, BasicHover));
                            painter->drawRoundedRect(toolButtonRect, _pBorderRadius, _pBorderRadius);
                        }
                        else
                        {
                            ...
                            {
                                painter->setBrush(_pIsTransparent ? YyThemeColor(_themeMode, BasicHoverAlpha) : YyThemeColor(_themeMode, BasicHover));
                                painter->drawRoundedRect(toolButtonRect, _pBorderRadius, _pBorderRadius);
                            }
                            else
                            {
                                if (!_pIsTransparent)
                                {
                                    painter->setBrush(YyThemeColor(_themeMode, BasicBase));
                                    painter->drawRoundedRect(toolButtonRect, _pBorderRadius, _pBorderRadius);
                                    // 底边线绘制
                                    painter->setPen(YyThemeColor(_themeMode, BasicBaseLine));
...
QSize YyToolButtonStyle::sizeFromContents(ContentsType type, const QStyleOption* option, const QSize& size, const QWidget* widget) const
{
...
void YyToolButtonStyle::_drawIndicator(QPainter* painter, const QStyleOptionToolButton* bopt, const QWidget* widget) const
{
    if (bopt->features.testFlag(QStyleOptionToolButton::MenuButtonPopup))
    {
        ...
        {
            painter->setBrush(YyThemeColor(_themeMode, BasicIndicator));
            QPainterPath path;
        ...
        // 指示器
        painter->setBrush(bopt->state.testFlag(QStyle::State_Enabled) ? YyThemeColor(_themeMode, BasicText) : YyThemeColor(_themeMode, BasicTextDisable));
    ...
    else if (bopt->features.testFlag(QStyleOptionToolButton::HasMenu))
    {
        ...
        int indicatorWidth = painter->fontMetrics().horizontalAdvance(QChar((unsigned short)YyIconType::AngleDown));
        ...
        painter->setPen(YyThemeColor(_themeMode, BasicText));
        ...
        painter->drawText(expandIconRect, Qt::AlignCenter, QChar((unsigned short)YyIconType::AngleDown));
        painter->restore();
    }
}

void YyToolButtonStyle::_drawIcon(QPainter* painter, QRectF iconRect, const QStyleOptionToolButton* bopt, const QWidget* widget) const
{
    if (bopt->toolButtonStyle != Qt::ToolButtonTextOnly)
    {
        QSize iconSize = bopt->iconSize;
        if (widget->property("YyIconType").toString().isEmpty())
        {
            // 绘制QIcon
            ...
            if (!icon.isNull())
            {
                ...
                case Qt::ToolButtonFollowStyle://跟随父级控件
        ...
        else
        {
            ...
            {
                painter->setPen(YyThemeColor(_themeMode, BasicTextDisable));
            }
            QFont iconFont = QFont("ElaAwesome");
            int iconRotate = widget->property("YyIconRotate").toInt();
            switch (bopt->toolButtonStyle)
            {
            case Qt::ToolButtonIconOnly:
            {
                ...
                painter->drawText(iconRect, Qt::AlignCenter, widget->property("YyIconType").toString());
                break;
            }
            ...
            case Qt::ToolButtonTextBesideIcon:
            {
                ...
                painter->drawText(adjustIconRect, Qt::AlignCenter, widget->property("YyIconType").toString());
                break;
            }
            case Qt::ToolButtonTextUnderIcon:
            {
                ...
                painter->drawText(adjustIconRect, Qt::AlignHCenter, widget->property("YyIconType").toString());
                break;
            }
            default:
            {
...
void YyToolButtonStyle::_drawText(QPainter* painter, QRect contentRect, const QStyleOptionToolButton* bopt) const
{
...
qreal YyToolButtonStyle::_calculateExpandIndicatorWidth(const QStyleOptionToolButton* bopt, QPainter* painter) const
{
    // 展开指示器
    QSize iconSize = bopt->iconSize;
    painter->save();
