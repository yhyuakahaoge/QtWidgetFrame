...
#include "YyTheme.h"
YyTabBarStyle::YyTabBarStyle(QStyle* style)
{
...
YyTabBarStyle::~YyTabBarStyle()
{
}

void YyTabBarStyle::drawPrimitive(PrimitiveElement pe, const QStyleOption* opt, QPainter* p, const QWidget* w) const
{
    // qDebug() << pe << opt->rect;
    switch (pe)
    {
    ...
    case QStyle::PE_IndicatorTabClose:
    {
        ...
        {
            p->setPen(Qt::NoPen);
            p->setBrush(YyThemeColor(_themeMode, BasicHoverAlpha));
            p->drawRoundedRect(opt->rect, 2, 2);
        }
        QFont iconFont = QFont("ElaAwesome");
        iconFont.setPixelSize(16);
        p->setFont(iconFont);
        p->setPen(YyThemeColor(_themeMode, BasicText));
        p->drawText(opt->rect, Qt::AlignCenter, QChar((unsigned short)YyIconType::Xmark));
        p->restore();
        return;
    }
    default:
...
void YyTabBarStyle::drawControl(ControlElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const
{
    int margin = 9;
    int topRadius = 7;
    switch (element)
    ...
    case QStyle::CE_TabBarTabShape:
    {
        ...
        if (const QStyleOptionTab* topt = qstyleoption_cast<const QStyleOptionTab*>(option))
        {
            ...
            if (topt->state.testFlag(QStyle::State_Selected))
            {
                ...
                painter->setBrush(topt->state.testFlag(QStyle::State_Sunken) ? YyThemeColor(_themeMode, BasicSelectedHover) : YyThemeColor(_themeMode, BasicSelectedAlpha));
                QPainterPath path;
            ...
            else
            {
                ...
                {
                    painter->setBrush(YyThemeColor(_themeMode, BasicHoverAlpha));
                }
                else
                {
...
QSize YyTabBarStyle::sizeFromContents(ContentsType type, const QStyleOption* option, const QSize& size, const QWidget* widget) const
{
    switch (type)
    {
    case CT_TabBarTab:
...
QRect YyTabBarStyle::subElementRect(SubElement element, const QStyleOption* option, const QWidget* widget) const
{
    switch (element)
    {
