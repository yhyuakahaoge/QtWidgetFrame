...
YyWinShadowHelper::YyWinShadowHelper(QObject* parent)
    : QObject(parent)
{
...
YyWinShadowHelper::~YyWinShadowHelper()
{
}

bool YyWinShadowHelper::initWinAPI()
{
...
void YyWinShadowHelper::setWindowShadow(quint64 hwnd)
{
...
void YyWinShadowHelper::setWindowDisplayMode(QWidget* widget, YyApplicationType::WindowDisplayMode displayMode, YyApplicationType::WindowDisplayMode lastDisplayMode)
{
    HWND winHwnd = (HWND)widget->winId();
    switch (lastDisplayMode)
    {
    case YyApplicationType::Mica:
    {
    ...
    case YyApplicationType::MicaAlt:
    {
        if (!compareWindowsVersion(Win11_22H2))
        {
            break;
    ...
    case YyApplicationType::Acrylic:
    {
        if (!compareWindowsVersion(Win11_Origin))
        {
            break;
    ...
    case YyApplicationType::DWMBlur:
    {
...
bool YyWinShadowHelper::getIsCompositionEnabled() const
{
...
bool YyWinShadowHelper::getIsFullScreen(const HWND& hwnd)
{
    RECT windowRect{};
...
MONITORINFOEXW YyWinShadowHelper::getMonitorForWindow(const HWND& hwnd)
{
...
quint32 YyWinShadowHelper::getResizeBorderThickness(const HWND& hwnd)
{
    return getSystemMetricsForDpi(hwnd, SM_CXSIZEFRAME) + getSystemMetricsForDpi(hwnd, SM_CXPADDEDBORDER);
}

quint32 YyWinShadowHelper::getDpiForWindow(const HWND& hwnd)
{
    if (_getDpiForWindow)
    {
...
int YyWinShadowHelper::getSystemMetricsForDpi(const HWND& hwnd, const int index)
{
...
bool YyWinShadowHelper::compareWindowsVersion(const QString& windowsVersion) const
{
...
void YyWinShadowHelper::_externWindowMargins(HWND hwnd)
{
