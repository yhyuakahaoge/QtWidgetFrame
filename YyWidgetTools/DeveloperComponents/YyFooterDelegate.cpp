...
#include "YyTheme.h"
YyFooterDelegate::YyFooterDelegate(QObject* parent)
    : QStyledItemDelegate{parent}
{
    _pYyListView = nullptr;
...
YyFooterDelegate::~YyFooterDelegate()
{
}

void YyFooterDelegate::navigationNodeStateChange(QVariantMap data)
{
...
void YyFooterDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
    QStyleOptionViewItem viewOption(option);
    initStyleOption(&viewOption, index);
    YyFooterModel* model = dynamic_cast<YyFooterModel*>(const_cast<QAbstractItemModel*>(index.model()));
    ...
    // 图标绘制
    painter->setPen(index == _pPressIndex ? YyThemeColor(_themeMode, BasicTextPress) : YyThemeColor(_themeMode, BasicText));
    ...
    if (keyPoints)
    {
        ...
        painter->setBrush(YyThemeColor(_themeMode, StatusDanger));
...
QSize YyFooterDelegate::sizeHint(const QStyleOptionViewItem& option, const QModelIndex& index) const
{
...
bool YyFooterDelegate::_compareItemY(YyNavigationNode* node1, YyNavigationNode* node2)
{
    if (!node1)
    {
        return true;
