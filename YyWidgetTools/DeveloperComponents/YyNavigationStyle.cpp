...
YyNavigationStyle::YyNavigationStyle(QStyle* style)
{
    _pOpacity = 1;
    _pItemHeight = 38;
...
YyNavigationStyle::~YyNavigationStyle()
{
}

void YyNavigationStyle::drawPrimitive(PrimitiveElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const
{
    switch (element)
    {
    case QStyle::PE_PanelItemViewItem:
    {
        ...
        if (const QStyleOptionViewItem* vopt = qstyleoption_cast<const QStyleOptionViewItem*>(option))
        {
            ...
            if (vopt->state & QStyle::State_Selected)
            {
                ...
                {
                    // 选中时点击
                    painter->fillPath(path, YyThemeColor(_themeMode, BasicHoverAlpha));
                }
                else
                {
                    ...
                    {
                        // 选中时覆盖
                        painter->fillPath(path, YyThemeColor(_themeMode, BasicSelectedHoverAlpha));
                    }
                    else
                    {
                        // 选中
                        painter->fillPath(path, YyThemeColor(_themeMode, BasicSelectedAlpha));
                    }
                }
            }
            else
...
void YyNavigationStyle::drawControl(ControlElement element, const QStyleOption* option, QPainter* painter, const QWidget* widget) const
{
    switch (element)
    {
    ...
    case QStyle::CE_ItemViewItem:
    {
        if (const QStyleOptionViewItem* vopt = qstyleoption_cast<const QStyleOptionViewItem*>(option))
        {
            ...
            YyNavigationNode* node = static_cast<YyNavigationNode*>(vopt->index.internalPointer());
            YyNavigationModel* model = dynamic_cast<YyNavigationModel*>(const_cast<QAbstractItemModel*>(vopt->index.model()));
            ...
            if (_isSelectMarkDisplay && (node == model->getSelectedNode() || node == model->getSelectedExpandedNode()))
            {
                ...
                painter->setBrush(YyThemeColor(_themeMode, PrimaryNormal));
            ...
            // 图标绘制
            painter->setPen(vopt->index == _pPressIndex ? YyThemeColor(_themeMode, BasicTextPress) : YyThemeColor(_themeMode, BasicText));
            if (node->getAwesome() != YyIconType::None)
            {
                painter->save();
            ...
            if (node->getIsCategoryNode())
            {
                ...
                painter->setPen(YyThemeColor(_themeMode, BasicTextCategory));
            }
            else
            {
                painter->setPen((vopt->index == _pPressIndex) ? YyThemeColor(_themeMode, BasicTextPress) : YyThemeColor(_themeMode, BasicText));
            ...
            if (viewWidth > _pNavigationView->getNavigationBarPrivate()->_pNavigationBarWidth - _indicatorIconAreaWidth)
            {
                ...
                if (node->getIsExpanderNode())
                {
                    if (node->getIsHasChild())
                    {
                        ...
                        painter->drawText(expandIconRect, Qt::AlignVCenter, QChar((unsigned short)YyIconType::AngleDown));
                        painter->restore();
                    }
                ...
                else
                {
                    ...
                    if (keyPoints)
                    {
                        ...
                        painter->setPen(QPen(YyThemeColor(_themeMode, BasicTextInvert), 2));
...
QSize YyNavigationStyle::sizeFromContents(ContentsType type, const QStyleOption* option, const QSize& size, const QWidget* widget) const
{
    switch (type)
    {
...
// 控制子导航弹出透明度 和指示器箭头方向
void YyNavigationStyle::navigationNodeStateChange(QVariantMap data)
{
    if (data.contains("Expand"))
    {
        YyNavigationNode* lastExpandNode = _expandAnimationTargetNode;
        _opacityAnimationTargetNode = data.value("Expand").value<YyNavigationNode*>();
    ...
    {
        YyNavigationNode* lastExpandNode = _expandAnimationTargetNode;
        _opacityAnimationTargetNode = data.value("Collapse").value<YyNavigationNode*>();
        _expandAnimationTargetNode = _opacityAnimationTargetNode;
        _pOpacity = 0;
    ...
    {
        _lastSelectedNode = data.value("LastSelectedNode").value<YyNavigationNode*>();
        YyNavigationNode* selectedNode = data.value("SelectedNode").value<YyNavigationNode*>();
...
bool YyNavigationStyle::_compareItemY(YyNavigationNode* node1, YyNavigationNode* node2)
{
    ...
    {
        YyNavigationNode* node1OriginalNode = node1->getOriginalNode();
        YyNavigationNode* node2OriginalNode = node2->getOriginalNode();
        // 不同父节点  相同起源节点
        if (node1OriginalNode == node2OriginalNode)
        {
            ...
            {
                YyNavigationNode* node1ParentNode = node1->getParentNode();
                YyNavigationNode* node2ParentNode = node2->getParentNode();
