...
#include "YyTheme.h"
YyScrollBarStyle::YyScrollBarStyle(QStyle* style)
{
    _pIsExpand = false;
    _pOpacity = 0;
    _pScrollBar = nullptr;
...
YyScrollBarStyle::~YyScrollBarStyle()
{
}

void YyScrollBarStyle::drawComplexControl(ComplexControl control, const QStyleOptionComplex* option, QPainter* painter, const QWidget* widget) const
{
    ...
    case QStyle::CC_ScrollBar:
    {
        if (const QStyleOptionSlider* sopt = qstyleoption_cast<const QStyleOptionSlider*>(option))
        {
            ...
            if (_pIsExpand)
            {
                ...
                int sideLength = 8;
                painter->setBrush(YyThemeColor(_themeMode, ScrollBarHandle));
...
int YyScrollBarStyle::pixelMetric(PixelMetric metric, const QStyleOption* option, const QWidget* widget) const
{
...
int YyScrollBarStyle::styleHint(StyleHint hint, const QStyleOption* option, const QWidget* widget, QStyleHintReturn* returnData) const
{
...
void YyScrollBarStyle::startExpandAnimation(bool isExpand)
{
    if (isExpand)
    {
        _pIsExpand = true;
